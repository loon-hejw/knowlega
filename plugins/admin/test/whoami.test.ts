...
test("a stalled core is bounded by the admin probe timeout", async () => {
  delayWhoami = true;
  ...
  assert.ok(elapsedMs < 1_750, `probe took ${elapsedMs}ms instead of timing out promptly`);
});
