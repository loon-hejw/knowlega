...
test("a scope mismatch is rejected before the turn request", async () => {
  let fetches = 0;
  ...
  await assert.rejects(
    runApprovalTurn(
      ...
      () => ({
        scopeId: "group:web-project-two",
        scopeGuard: {
          mountedThreadRef: "web:u:t",
          currentThreadRef: "web:u:t",
          mountedScopeId: "group:web-project-one",
          currentScopeId: "group:web-project-two",
          sessionScopeId: "group:web-project-one",
        },
      }),
      undefined,
    ),
