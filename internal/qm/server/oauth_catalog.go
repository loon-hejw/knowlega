...
var oauthCatalogProviders = []oauthCatalogProvider{
	...
	},
	{
		...
		SetupGuide: map[string]any{"console": "Slack API → Your Apps → OAuth & Permissions", "url": "https://api.slack.com/apps", "steps": []string{"Create a Slack app (from scratch) in your workspace.", "Under OAuth & Permissions, add the redirect URL shown below.", "Add the requested USER token scopes under 'User Token Scopes'.", "Install the app to the workspace.", "Add the Client ID and Client secret under qm.oauth.clients in the QM YAML file."}, "scopesRationale": "User-token scopes let the agent read/post, search, and edit canvases as the connecting user, not a bot."},
	},
	{
		Name: "notion", Hosts: []string{"api.notion.com"}, Scopes: []string{}, ConsentMode: "standard", RedirectPath: "notion/callback",
		SetupGuide: map[string]any{"console": "Notion → My integrations → New integration (Public)", "url": "https://www.notion.so/profile/integrations", "steps": []string{"Create a public OAuth integration.", "Add the redirect URI shown below to the integration's redirect URIs.", "Configure the capabilities the integration needs.", "Add the Client ID and Client secret under qm.oauth.clients in the QM YAML file."}, "scopesRationale": "Notion grants access per selected pages at consent time (no OAuth scope list)."},
	},
	{
		Name: "linear", Hosts: []string{"api.linear.app"}, Scopes: []string{"read", "write"}, ConsentMode: "standard", RedirectPath: "linear/callback",
		SetupGuide: map[string]any{"console": "Linear → Settings → API → OAuth applications", "url": "https://linear.app/settings/api/applications", "steps": []string{"Create an OAuth application.", "Add the redirect URI shown below as a callback URL.", "Select the read/write scopes.", "Add the Client ID and Client secret under qm.oauth.clients in the QM YAML file."}, "scopesRationale": "read/write let the agent query and update issues, projects, and comments."},
	},
	{
		...
		SetupGuide: map[string]any{"console": "Dropbox App Console → Create app", "url": "https://www.dropbox.com/developers/apps/create", "steps": []string{"Create an app: Scoped access, Full Dropbox access.", "On the app's OAuth 2 section, add the redirect URI shown below.", "On the Permissions tab, enable account_info.read, files.metadata.read, files.content.read, files.content.write, sharing.read, sharing.write — then Submit.", "Add the App key and App secret under qm.oauth.clients in the QM YAML file."}, "scopesRationale": "files.content/metadata read+write back browse/download/upload; sharing.read/write let the agent manage shared links; account_info.read identifies the account."},
	},
	{
		Name: "github", Hosts: []string{"api.github.com"}, Scopes: []string{"repo", "read:org"}, ConsentMode: "github_app", RedirectPath: "github/callback",
		SetupGuide: map[string]any{"console": "GitHub → Settings → Developer settings → OAuth Apps (or GitHub App)", "url": "https://github.com/settings/applications/new", "steps": []string{"Register a new OAuth App (or a GitHub App for installation tokens).", "Set the Authorization callback URL to the redirect URI shown below.", "Select the repo/org scopes the agent needs.", "Add the Client ID and Client secret under qm.oauth.clients in the QM YAML file."}, "scopesRationale": "repo/read:org back the GitHub skills (PRs, issues, org-visible repos) as the connecting user."},
	},
	{
		...
		SetupGuide: map[string]any{"console": "X Developer Portal → Projects & Apps → your app → User authentication settings", "url": "https://developer.x.com/en/portal/projects-and-apps", "steps": []string{"Set up User authentication: OAuth 2.0, type Confidential client, App permissions Read and write.", "Add the Callback URI shown below to the app's Callback URLs.", "Under Keys and tokens, generate the OAuth 2.0 Client ID and Client Secret.", "Add the Client ID and Client secret under qm.oauth.clients in the QM YAML file."}, "scopesRationale": "tweet.read/users.read back reads; tweet.write lets the connecting user post as themselves; offline.access issues a refresh token so the 2-hour access token renews."},
	},
}
...
func (h *HTTPServer) configuredOAuthProviders() map[string]bool {
	configured := make(map[string]bool, len(h.config.QM.OAuth.Clients))
	for _, client := range h.config.QM.OAuth.Clients {
		configured[strings.TrimSpace(client.Provider)] = true
	}
	return configured
}
