...
owned by this plugin. The browser uses one frontend process:

- **The unified `node:http` server** (`server/unified.ts`) — holds the signed-in principal
...
- The same process mounts the Portal at the origin root, the Web UI at `/`, and the Admin
  surface at `/admin/`. Portal authentication and authorization stay server-side; the Go
  QM backend remains the only backend service.
...
Dev (HMR): run `WEB_UI_DEV=1 PORT=8129 npm run serve`. The unified server embeds Vite
middleware, so user and admin routes keep the same origin while the browser uses HMR.

Env (see `.env.example`): `CORE_API_URL` (default `http://localhost:18083`),
`CORE_ORG_ID` (default `acme`), `PORT` (default 8129), `PORTAL_PUBLIC_URL`,
`PORTAL_SESSION_SECRET`, `ADMIN_BASE_PATH` (default `/admin`), `WEB_UI_PUBLIC_URL`,
...
This is a **surface plugin**: it carries its own front-end deps (Vite, lit, pi-web-ui),
while the zero-runtime-dep Go backend remains separate from the browser process.
