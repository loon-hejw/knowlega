...
): Promise<{ isAdmin: boolean; role?: string; scopeId?: string } | null> {
  const corePath = "/v1/admin/whoami";
  try {
    const r = await fetch(`${CORE}${corePath}`, {
      headers: {
        ...signedHeaders("GET", corePath, ""),
        ...(identityToken ? { [PORTAL_IDENTITY_HEADER]: identityToken } : portalIdentityHeader()),
        "x-admin-actor": `${principal}@${ORG}`,
      },
      // Keep the embedded probe bounded just like Portal's standalone
      // upstream probe. A stalled core must become an outage response,
      // rather than holding the front door request open indefinitely.
...
export async function probeAdmin(
  ...
  const who = await coreWhoami(principal, identityToken);
  return who ? { isAdmin: who.isAdmin, failed: false } : { isAdmin: false, failed: true };
}
...
async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const raw = req.headers[PORTAL_IDENTITY_HEADER];
  const token = Array.isArray(raw) ? raw[0] : raw;
  return portalTokenStore.run(token, () => handleRequest(req, res));
}
