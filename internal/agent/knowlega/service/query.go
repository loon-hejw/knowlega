...
// QueryAnswerStore persists the complete answer payload behind a query id so
// writeback remains available after a QM backend restart. QueryLogStore is
// still used for lightweight operational telemetry.
type QueryAnswerStore interface {
	SaveQueryAnswerPayload(context.Context, string, string, []byte) error
	LoadQueryAnswerPayload(context.Context, string, string) ([]byte, error)
}
