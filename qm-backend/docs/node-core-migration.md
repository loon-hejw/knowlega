...
`qm-backend` also exposes the existing `knowledge.v1.KnowledgeCore` gRPC
contract used by QM. Point `KNOWLEDGE_CORE_GRPC_ADDR` at the Go gRPC listener
and set `KNOWLEDGE_CORE_AUTH_TOKEN` to the same value as
`QM_BACKEND_GRPC_INTERNAL_TOKEN`. Scope initialization, Markdown workspace
navigation, safe document reads, candidate search, and LLM-backed query are
served by Go. Enable query by configuring `knowledge.llm.base_url`,
`knowledge.llm.api_key`, and `knowledge.llm.model` in the Go YAML config.
The extended `IngestSource` RPC compiles immutable source content into wiki
pages through the same configured LLM; it rejects missing provenance, missing
navigation updates, invalid wiki paths, or uncross-linked output. Existing
pages are archived below `.kbcore/page-versions/` before replacement.
`Query` returns a durable `query_id`, `query_plan`, and `can_write_back` flag;
only a query with read non-navigation evidence can be saved through
`SaveQueryAnswer`, which writes a versioned page under `wiki/syntheses/` and
updates index and overview navigation.
The LLM query runtime follows a bounded planner loop: it reads navigation
first, then permits at most six `read`, `search`, or `final` actions. Search
only produces candidates; the runtime reads those candidates before synthesis
and refuses a final answer based solely on navigation documents.
