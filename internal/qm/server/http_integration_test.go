...
func TestProjectRouteCompatibility(t *testing.T) {
	...
	knowledgeAgent, err := knowlega.New(knowlega.AgentOptions{RootDir: t.TempDir()})
	if err != nil {
		t.Fatal(err)
	}
	...
	knowledgeStatus, err := knowledgeAgent.Status(knowlega.ScopeRef{OrgID: "acme", ExternalScopeID: biz.ProjectScopeID(created.Project.ID), Kind: "project", Name: "Roadmap 2026"})
	if err != nil || knowledgeStatus.Scope.Name != "Roadmap 2026" {
