...
test("a running snapshot that omits result stays open until the terminal snapshot", async () => {
  instantSleep();
  stubRuns([
    { status: "running", partial: "draft" },
    { status: "done", result: { status: "ok", reply: "draft complete" }, partial: "draft" },
  ]);

  const stream = createAssistantMessageEventStream();
  const partial = blankAssistant();
  const st = freshAcc(Date.now());

  await pollRun(stream, partial, "run-omitted-result", st);
  const final = await drain(stream);

  assert.equal(final.stopReason, "stop");
  const block = final.content[0];
  assert.equal(block?.type === "text" ? block.text : "", "draft complete");
});
...
test("an incomplete terminal run is not rendered as completed and still has a visible fallback", async () => {
	globalThis.fetch = (async () => {
		throw new Error("incomplete terminal snapshot should not poll");
	}) as typeof fetch;

	const streamFn = makeRunResumeStreamFn("run-incomplete", {
		status: "done",
		result: { status: "incomplete", completionStatus: "incomplete", reason: "execution budget reached" },
		partial: "",
		activity: [],
		startedAt: 100,
		finishedAt: 200,
	...
	assert.equal(block?.type === "text" ? block.text : "", "Execution incomplete: execution budget reached");
	assert.equal(work?.status, "failed");
	assert.equal(work?.completionStatus, "incomplete");
});
