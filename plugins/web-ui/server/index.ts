...
function sseEvent(res: ServerResponse, event: string, data: unknown, id?: number): void {
...
export async function initializeEmbeddedWebUI(server: Server): Promise<void> {
...
const routeRequest = async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (path === "/me" || path.startsWith("/api/")) {
    ...
    const projectKnowledgeDocument = path.match(/^\/api\/projects\/([^/]+)\/knowledge\/documents$/);
    if (method === "GET" && projectKnowledgeDocument) {
      const id = decodeURIComponent(projectKnowledgeDocument[1]!);
      const documentPath = url.searchParams.get("path")?.trim() ?? "";
      if (!documentPath) return json(res, 400, { error: "bad_request", message: "path required" });
      const r = await coreFetch(
        "GET",
        `/v1/projects/${encodeURIComponent(id)}/knowledge/documents?viewer=${encodeURIComponent(user)}&path=${encodeURIComponent(documentPath)}`,
      );
      return relay(res, r);
    }
    ...
    const addProjectFile = path.match(/^\/api\/projects\/([^/]+)\/files$/);
    if (method === "POST" && addProjectFile) {
      const id = decodeURIComponent(addProjectFile[1]!);
      let fileId = "";
      try {
        const p = JSON.parse((await readBody(req)) || "{}") as { fileId?: unknown };
        if (typeof p.fileId === "string") fileId = p.fileId.trim();
      } catch (e) {
        if (e instanceof PayloadTooLargeError) throw e;
        return json(res, 400, { error: "bad_request" });
      }
      if (!fileId) return json(res, 400, { error: "bad_request", message: "fileId required" });
      const r = await coreFetch(
        "POST",
        `/v1/projects/${encodeURIComponent(id)}/files`,
        JSON.stringify({ principalId: user, fileId }),
      );
      return relay(res, r);
    }

    const retryProjectFile = path.match(/^\/api\/projects\/([^/]+)\/files\/([^/]+)\/retry$/);
    if (method === "POST" && retryProjectFile) {
      const id = decodeURIComponent(retryProjectFile[1]!);
      const fileId = decodeURIComponent(retryProjectFile[2]!);
      const r = await coreFetch(
        "POST",
        `/v1/projects/${encodeURIComponent(id)}/files/${encodeURIComponent(fileId)}/retry`,
        JSON.stringify({ principalId: user }),
      );
      return relay(res, r);
    }

    const removeProjectFile = path.match(/^\/api\/projects\/([^/]+)\/files\/([^/]+)$/);
    if (method === "DELETE" && removeProjectFile) {
      const id = decodeURIComponent(removeProjectFile[1]!);
      const fileId = decodeURIComponent(removeProjectFile[2]!);
      const r = await coreFetch(
        "DELETE",
        `/v1/projects/${encodeURIComponent(id)}/files/${encodeURIComponent(fileId)}`,
        JSON.stringify({ principalId: user }),
      );
      return relay(res, r);
    }
    ...
    if (method === "GET" && path.startsWith("/api/runs/") && path.endsWith("/events")) {
      ...
      let cursor = Number.parseInt(String(req.headers["last-event-id"] ?? ""), 10);
      if (!Number.isFinite(cursor) || cursor < 0) cursor = -1;
      const activityBySeq = new Map<number, unknown>();
      ...
      for (;;) {
        ...
        try {
          r = await coreFetch("GET", `/v1/runs/${encodeURIComponent(id)}${cursor >= 0 ? `?after=${cursor}` : ""}`);
        } catch {
          try {
            r = await coreFetch("GET", `/v1/runs/${encodeURIComponent(id)}${cursor >= 0 ? `?after=${cursor}` : ""}`);
          } catch {
        ...
        if (typeof run.attempt === "number" && run.attempt > 0 && run.attempt !== attempt) {
        ...
        const chunk = typeof run.partial === "string" ? run.partial : "";
        const partial = run.partialMode === "append" ? acc + chunk : chunk;
        const activity = Array.isArray(run.activity) ? run.activity : [];
        if (run.partialReset === true || attemptReset || partial.length > acc.length) {
          acc = partial;
          sseEvent(
            res,
            "partial",
            { partial: acc, attempt, reset: run.partialReset === true || attemptReset },
            run.eventCursor,
          );
          lastProgressAt = now;
          lastBeat = now;
        ...
        if (activity.length) {
          for (const item of activity) {
            ...
            if (typeof seq === "number") activityBySeq.set(seq, item);
          }
          sseEvent(res, "activity", { activity, startedAt: run.startedAt ?? null, attempt }, run.eventCursor);
          lastProgressAt = now;
          lastBeat = now;
        }
        if (typeof run.eventCursor === "number" && run.eventCursor > cursor) cursor = run.eventCursor;
        if (parsed) {
        ...
        if (terminal || run.replyComplete) {
          ...
          sseEvent(res, "done", {
            ...
            partial: acc,
            activity: [...activityBySeq.values()],
            attempt,
            eventCursor: cursor >= 0 ? cursor : undefined,
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  initializeEmbeddedWebUI(server)
    .then(() => {
      server.listen(PORT, () => {
        console.log(
