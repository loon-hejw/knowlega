...
export function fieldSelect(props: {
  ...
  >
    <select
      ...
      class=${props.selectClassName ?? nothing}
...
export function relTime(ms: number): string {
  ...
  if (s < 3600) return getLocale() === "zh-CN" ? `${Math.floor(s / 60)} 分钟前` : `${Math.floor(s / 60)}m ago`;
  if (s < 86400) return getLocale() === "zh-CN" ? `${Math.floor(s / 3600)} 小时前` : `${Math.floor(s / 3600)}h ago`;
  return getLocale() === "zh-CN" ? `${Math.floor(s / 86400)} 天前` : `${Math.floor(s / 86400)}d ago`;
}
