  ...
  return html`
    ${
      r.files.length
        ? resourceGroup(
            "Files",
            r.files.map((file) => fileRow(file, manage)),
          )
        : nothing
    }
    ${
...
function fileRow(f: ScopeFile, manage: boolean): TemplateResult {
  const busy = resourceBusy.has(`file:${f.id}`);
  return html`
    <div class="context-session-row context-resource-row">
      ...
      <span class="context-session-meta">
        ...
        ${
          manage && f.projectFile
            ? html`<button
                class="context-resource-action danger"
                type="button"
                ?disabled=${busy}
                @click=${() => void deleteProjectFile(f)}
              >
...
async function deleteProjectFile(file: ScopeFile): Promise<void> {
  const projectId = file.projectFile?.projectId;
  const key = `file:${file.id}`;
  if (!projectId || resourceBusy.has(key)) return;
  if (!confirm(`Delete ${file.name} from this project and update its knowledge base?`)) return;
  resourceBusy.add(key);
  drawContexts();
  try {
    ...
    contextsState.resourcesNotice = errMessage(e, "Couldn't delete that file.");
  } finally {
    resourceBusy.delete(key);
    drawContexts();
  }
