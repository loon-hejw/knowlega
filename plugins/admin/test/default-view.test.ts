...
test("connector setup is a read-only YAML status projection", () => {
  assert.match(html, /api\("GET", "\/api\/connector-catalog"\)/);
  assert.match(html, /api\("GET", "\/api\/onboarding"\)/);
  assert.match(html, /Configured in YAML/);
  assert.match(html, /Read-only · YAML managed/);
  assert.doesNotMatch(html, /const CONNECTOR_CATALOG = \[/);
  assert.doesNotMatch(html, /id="slack-bot-token"/);
  assert.doesNotMatch(html, /api\("PUT", "\/api\/slack-installation"/);
  assert.doesNotMatch(html, /id="conn-client-secret"/);
});

test("onboarding summarizes YAML-owned model, Slack, and OAuth status", () => {
  const onboarding = html.slice(html.indexOf('id="view-onboarding"'), html.indexOf('id="view-governance"'));
  assert.match(onboarding, /view-onboarding/);
  assert.match(onboarding, /Base model/);
  assert.match(html, /api\("GET", "\/api\/onboarding"\)/);
  assert.match(onboarding, /qm\.models/);
  assert.doesNotMatch(html, /api\("GET", "\/api\/model-providers"\)/);
  assert.doesNotMatch(html, /api\("PUT", "\/api\/model-providers/);
  assert.doesNotMatch(onboarding, /api-key|API key/i);
