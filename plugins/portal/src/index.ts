...
export type EmbeddedSurfaceHandler = (
  req: IncomingMessage,
  res: ServerResponse,
  context: EmbeddedSurfaceContext,
) => void | Promise<void>;

export type EmbeddedAdminProbe = (
  principal: string,
  identityToken?: string,
) => Promise<{ isAdmin: boolean; failed?: boolean }>;

let embeddedSurfaceHandlers: Partial<Record<"web-ui" | "admin", EmbeddedSurfaceHandler>> = {};
let embeddedAdminProbe: EmbeddedAdminProbe | undefined;

export function configureEmbeddedSurfaceHandlers(
  handlers: Partial<Record<"web-ui" | "admin", EmbeddedSurfaceHandler>> & { adminProbe?: EmbeddedAdminProbe },
): void {
  const { adminProbe, ...surfaces } = handlers;
  embeddedSurfaceHandlers = { ...surfaces };
  embeddedAdminProbe = adminProbe;
}
...
async function adminProbe(sub: string): Promise<{ isAdmin: boolean; failed: boolean }> {
  ...
  const identityToken = PORTAL_IDENTITY_SECRET
    ? mintPortalIdentity({ p: sub, exp: Date.now() + 60_000 }, PORTAL_IDENTITY_SECRET)
    : undefined;
  if (embeddedAdminProbe) {
    try {
      const result = await embeddedAdminProbe(sub, identityToken);
      if (result.failed) return { isAdmin: false, failed: true };
      adminCache.set(sub, result.isAdmin);
      return { isAdmin: result.isAdmin, failed: false };
    } catch {
  ...
  try {
    ...
    if (identityToken) headers[PORTAL_IDENTITY_HEADER] = identityToken;
...
async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (
    (key === "web-ui" || key === "admin") &&
    (await dispatchEmbeddedSurface(
      key,
      req,
      res,
      forwardPath,
      url.search,
      principal,
      !impersonator && session.name ? session.name : undefined,
      impersonator,
    ))
  ) {
    return;
...
async function dispatchEmbeddedSurface(
  key: "web-ui" | "admin",
  req: IncomingMessage,
  res: ServerResponse,
  forwardPath: string,
  ...
  const handler = embeddedSurfaceHandlers[key];
  ...
  const identityToken = PORTAL_IDENTITY_SECRET
    ...
    : undefined;
  const cookieName = COOKIE_FOR[key]!;
  const cookie = [
    `${cookieName}=${encodeURIComponent(principal)}`,
    ...(displayName ? [`${cookieName}_name=${encodeURIComponent(displayName)}`] : []),
    ...(impersonator ? [`webui_impersonator=${encodeURIComponent(impersonator)}`] : []),
  ].join("; ");
  ...
  req.headers = {
    ...originalHeaders,
    cookie,
    ...(identityToken ? { [PORTAL_IDENTITY_HEADER]: identityToken } : {}),
  };
  req.url = `${forwardPath}${search}`;
  try {
