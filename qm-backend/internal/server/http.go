...
func NewHTTPServer(cfg config.Config, pg *data.Postgres, knowledgeEngine *knowledge.Engine, logger *slog.Logger) (*khttp.Server, error) {
...
func (h *HTTPServer) ensureProjectKnowledge(ctx context.Context, project biz.Project, actor string) {
	...
	if _, err := h.knowledge.EnsureScope(ctx, knowledge.ScopeRef{OrgID: project.OrgID, ExternalScopeID: biz.ProjectScopeID(project.ID), Kind: "project", Name: project.Name}); err != nil {
