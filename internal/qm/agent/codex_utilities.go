...
func (a *CodexAdapter) OneShot(ctx context.Context, input UtilityInput) (string, error) {
	return a.codexUtilityCall(ctx, input, utilityModel(input.Model, a.harness.DefaultModel))
}

func (a *CodexAdapter) Judge(ctx context.Context, input UtilityInput) (string, error) {
	model := utilityModel(a.harness.Runtime.JudgeModel, utilityModel(input.Model, a.harness.DefaultModel))
	return a.codexUtilityCall(ctx, input, model)
}

func (a *CodexAdapter) codexUtilityCall(ctx context.Context, input UtilityInput, model string) (string, error) {
	sequence := 0
	result, err := a.RunTurn(ctx, TurnInput{
		...
		ScopeLabel: firstNonEmpty(input.ScopeLabel, "org:oneshot"), OrgScopeID: "org:oneshot", Harness: "codex", Model: model, ReadOnly: true,
...
func (*CodexAdapter) ShouldRespond(context.Context, DetectInput) DetectResult {
	return DetectResult{Respond: false, Reason: "codex does not expose shouldRespond"}
}

func (*CodexAdapter) CompactHistory(_ context.Context, input CompactInput) string {
	return deterministicPiCompact(input.History)
}

func (a *CodexAdapter) ContextTokenBudget(model string) (int, bool) {
...
func (a *CodexAdapter) ScreenSecurity(ctx context.Context, input UtilityInput) (*SecurityVerdict, error) {
...
func (*CodexAdapter) PickAckEmoji(context.Context, UtilityInput, []string) (string, error) {
	return "", nil
}

func (a *CodexAdapter) GenerateTitle(ctx context.Context, input UtilityInput) (string, error) {
...
func (a *CodexAdapter) SummarizeApproval(ctx context.Context, input UtilityInput, command, reason, purpose string) (string, error) {
