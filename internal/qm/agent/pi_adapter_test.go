...
func TestPiAdapterAllowsRepeatedIdenticalToolActions(t *testing.T) {
	transport := &fakePiTransport{completions: []PiCompletion{
	...
	if err != nil || result.Reply != "完成。" || len(tools.calls) != 3 {
	...
	if stalled {
		t.Fatalf("unexpected repeated-action redirect: %+v", progress)
	}
}
