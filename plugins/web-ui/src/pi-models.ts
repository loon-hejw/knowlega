...
export function getBaseModel(id: string, fallback?: { name: string; provider: string; protocol?: string }): PiModel {
  ...
  if (fallback) {
    const anthropic = fallback.protocol === "anthropic-messages" || fallback.protocol === "anthropic";
    const template = anthropic
      ? builtinModel("claude-sonnet-4-6")
      : (builtinModel("gpt-5.5") ??
