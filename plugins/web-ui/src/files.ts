...
import { File, Image, Plus, RefreshCw, Trash2, Upload, X } from "lucide";
...
interface FileItem {
  ...
  projectFile?: {
    projectId: string;
    status: "queued" | "processing" | "ready" | "failed" | "unsupported" | "removing";
...
let filesAttachOpen = false;
let filesAttachLoading = false;
let filesAttachChoices: FileRow[] = [];
...
function selectedProjectId(): string | null {
  ...
  return filesScope?.startsWith(prefix) ? filesScope.slice(prefix.length) : null;
}
...
function attachExistingChoices() {
  if (filesAttachLoading) return html`<div class="status">Loading files…</div>`;
  if (!filesAttachChoices.length) return html`<div class="empty compact">No other QM files are available.</div>`;
  return html`<div class="context-session-list">
    ${filesAttachChoices.map(
      (file) =>
        html`<div class="context-session-row context-resource-row">
          <span class="context-session-title">${file.name}</span>
          <span class="context-session-meta">
            <span>${formatBytes(file.sizeBytes)}</span>
            <button
              class="btn compact"
              type="button"
              ?disabled=${filesBusy.has(file.id)}
              @click=${() => void attachExistingFile(file)}
            >
              ${icon(Plus, 13)}<span>Add</span>
            </button>
          </span>
        </div>`,
    )}
...
function drawFiles(loading = false): void {
  ...
  render(
    html`
          ...
          ${scopeFilterControl(filesScope, (s) => {
            ...
            selectedProjectId()
              ? html`<button
                  ...
                  @click=${() => void toggleAttachFiles()}
                >
                  ${icon(filesAttachOpen ? X : Plus, 15)}<span>${filesAttachOpen ? "Close" : "Add existing"}</span>
                </button>`
              : nothing
      ...
      ${
        filesAttachOpen && selectedProjectId()
          ? html`<section class="context-panel context-resource-group">
              <div class="context-panel-heading context-resource-heading">
                <h2 class="context-panel-title">Add an existing QM file</h2>
              </div>
              ${attachExistingChoices()}
            </section>`
          : nothing
      }
...
async function toggleAttachFiles(): Promise<void> {
  filesAttachOpen = !filesAttachOpen;
  if (!filesAttachOpen) {
    drawFiles();
    return;
  }
  filesAttachLoading = true;
  drawFiles();
  try {
    const response = await api<{ owned?: FileItem[]; shared?: FileItem[] }>("/api/files?limit=200");
    const current = new Set(fileRows.map((file) => file.id));
    filesAttachChoices = rowsFromPage(response).filter((file) => !current.has(file.id) && file.openable);
  } catch (e) {
    filesNotice = errMessage(e, "Couldn't load existing files.");
    filesAttachChoices = [];
  } finally {
    filesAttachLoading = false;
    drawFiles();
  }
}

async function attachExistingFile(file: FileRow): Promise<void> {
  const projectId = selectedProjectId();
  if (!projectId || filesBusy.has(file.id)) return;
  filesBusy.add(file.id);
  drawFiles();
  try {
    ...
    filesAttachChoices = filesAttachChoices.filter((candidate) => candidate.id !== file.id);
    filesNotice = `${file.name} added to this project.`;
    await loadFiles(appState.viewRenderSeq);
  } catch (e) {
    filesNotice = errMessage(e, "Couldn't add that file.");
  } finally {
    filesBusy.delete(file.id);
    drawFiles();
  }
}
...
function fileRow(f: FileRow) {
  ...
  const projectId = f.projectFile?.projectId;
  const busy = filesBusy.has(f.id);
  return html`<article class="list-row file-row">
    ...
    <span class="list-row-meta"
      >${scopeChip(fileScope(f))}<span class="badge">${f.kind}</span>${
        f.projectFile
          ? html`<span class="badge" title=${f.projectFile.lastError ?? ""}>Knowledge: ${f.projectFile.status}</span>${
                f.projectFile.generatedPageCount
                  ? html`<span>${f.projectFile.generatedPageCount} pages</span>`
                  : nothing
              }`
          : nothing
      }<span>${formatBytes(f.sizeBytes)}</span
      ><span>${relTime(f.createdAt)}</span
      >${f.openable ? html`<a class="btn compact" href=${contentUrl} target="_blank" rel="noreferrer">Open</a>` : html`<span>Unavailable</span>`}${
        projectId && f.projectFile?.status === "failed"
          ? html`<button
              class="btn compact"
              type="button"
              ?disabled=${busy}
              @click=${() => void retryFile(projectId, f.id)}
            >
              ${icon(RefreshCw, 14)}<span>Retry</span>
            </button>`
          : nothing
      }${
        projectId
          ? html`<button
              ...
              type="button"
              ?disabled=${busy}
              @click=${() => void deleteProjectFile(projectId, f)}
            >
...
async function retryFile(projectId: string, fileId: string): Promise<void> {
  if (filesBusy.has(fileId)) return;
  filesBusy.add(fileId);
  drawFiles();
  try {
    await api(`/api/projects/${encodeURIComponent(projectId)}/files/${encodeURIComponent(fileId)}/retry`, {
      method: "POST",
    });
    filesNotice = "File queued for knowledge processing.";
    await loadFiles(appState.viewRenderSeq);
  } catch (e) {
    filesNotice = errMessage(e, "Couldn't retry that file.");
  } finally {
    filesBusy.delete(fileId);
    drawFiles();
  }
}

async function deleteProjectFile(projectId: string, file: FileRow): Promise<void> {
  if (filesBusy.has(file.id) || !confirm(`Delete ${file.name} from this project and update its knowledge base?`))
    return;
  filesBusy.add(file.id);
  drawFiles();
  try {
    await api(`/api/projects/${encodeURIComponent(projectId)}/files/${encodeURIComponent(file.id)}`, {
      method: "DELETE",
    });
    filesNotice = "File removed and knowledge base updated.";
    await loadFiles(appState.viewRenderSeq);
  } catch (e) {
