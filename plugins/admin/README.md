...
spec §14, delivered as an _added_ plugin (like the Slack plugin). In the normal
dev/runtime path it is mounted inside the unified frontend at `/admin/`, so the
browser has one origin and one frontend process. The direct `npm start` entry below
remains available for isolated development/tests. Either mode talks to the core
**only** over the admin governance API; the core has zero dependency on it.
...
The dev supervisor starts the unified frontend (`http://localhost:8129/`) and mounts
this surface at `/admin/`. To run the surface by itself for an isolated check:
...
cd plugins/admin
CORE_API_URL=http://localhost:18083 CORE_ORG_ID=acme PORT=8090 npm start

```

No build step, no runtime dependencies (pure `node:http` + native TS). Node 24+.

Env: `CORE_API_URL` (default `http://localhost:18083`), `CORE_ORG_ID` (default `acme`),
