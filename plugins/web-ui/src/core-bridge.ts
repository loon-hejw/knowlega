...
export interface TurnOptions {
  ...
  channelName?: string | null;
  scopeGuard?: {
    mountedThreadRef: string;
    currentThreadRef: string | null;
    mountedScopeId: string | null;
...
async function drive(
  ...
  try {
    ...
    validateTurnScope(threadRef, turnOptions);
    const thinkingLevel =
...
function validateTurnScope(threadRef: string, options: TurnOptions): void {
  const guard = options.scopeGuard;
  if (!guard) return;
  if (threadRef !== guard.mountedThreadRef || guard.currentThreadRef !== guard.mountedThreadRef) {
    throw new Error(
      "This conversation is no longer attached to its original project. Reopen it from the correct project before sending.",
    );
  }
  const mountedScope = normalizedTurnScope(guard.mountedScopeId);
  const scopes = [options.scopeId, guard.currentScopeId];
  if (guard.sessionScopeId !== undefined) scopes.push(guard.sessionScopeId);
  if (scopes.some((scope) => normalizedTurnScope(scope) !== mountedScope)) {
    throw new Error(
      "This conversation is no longer attached to its original project. Reopen it from the correct project before sending.",
    );
  }
...
function normalizedTurnScope(scopeId: string | null | undefined): string | null {
...
function mergeWork(work: WorkBlock | undefined, run: { activity?: unknown[]; startedAt?: number | null }): boolean {
  ...
  if (activity.length) {
    const merged = new Map(work.activity.map((item) => [item.seq, item]));
    for (const item of activity) merged.set(item.seq, item);
    const next = [...merged.values()].sort((a, b) => a.seq - b.seq);
    if (next.length !== work.activity.length || next.some((item, index) => item !== work.activity[index])) {
      work.activity = next;
      changed = true;
    }
  }
  if (activity.some((item) => item.type === "phase")) {
    changed = true;
  }
  for (const item of activity) {
    if (item.type !== "progress" && item.type !== "phase") continue;
    ...
    const fields: Array<[keyof WorkBlock, unknown]> = [
      ...
      ["toolLimit", payload.toolLimit],
      ["fallbackModel", payload.fallbackModel],
    ];
    for (const [key, value] of fields) {
      if (value === undefined || value === null || value === "") continue;
      if (work[key] !== value) {
        (work[key] as unknown) = value;
        changed = true;
      }
    }
  }
...
function applyRun(
  ...
  if (typeof run.eventCursor === "number") st.eventCursor = run.eventCursor;
  if (typeof run.attempt === "number" && run.attempt > 0 && run.attempt !== st.attempt) {
    if (st.attempt !== undefined && run.attempt > st.attempt) resetPartial(stream, partial, st);
    st.attempt = run.attempt;
    if (work) work.attempt = run.attempt;
  }
  const incoming = typeof run.partial === "string" ? run.partial : "";
  if (run.partialReset === true) resetPartial(stream, partial, st);
  const p = run.partialMode === "append" ? st.acc + incoming : incoming;
  if (p.length > st.acc.length) {
    st.lastProgressAt = now();
    pushDelta(stream, partial, st, p);
  }
  const terminal = run.status === "done" || run.status === "failed" || run.result != null || run.replyComplete === true;
  if (!terminal) return "open";
  const res = run.result;
  if (work && res) {
    const resultFields: Array<[keyof WorkBlock, unknown]> = [
      ...
      ["modelLimit", res.modelLimit],
      ["toolLimit", res.toolLimit],
      ["fallbackModel", res.fallbackModel],
    ];
    for (const [key, value] of resultFields) {
      if (value !== undefined && value !== null && value !== "") (work[key] as unknown) = value;
    }
  }
  ...
  if (work) {
    ...
    const resultIncomplete = res != null && (res.status === "incomplete" || res.completionStatus === "incomplete");
    const resultFailed = res != null && ["error", "failed"].includes(res.status);
    work.status =
      !paused && !approvalDenied && !quiet && (run.status === "failed" || resultFailed || resultIncomplete)
        ? "failed"
        : "complete";
  ...
  if (res?.reply || res?.partial) {
    finish(stream, partial, st, res.reply ?? res.partial ?? "");
    return "terminal";
  }
  if (approvalDenied) {
  ...
  if (
    ...
    !quiet &&
    (run.status === "failed" ||
      (res && res.status !== "ok" && res.status !== "incomplete" && res.completionStatus !== "incomplete"))
  ) {
  ...
  const fallbackText =
    paused || quiet
      ? ""
      : res?.completionStatus === "incomplete" || res?.status === "incomplete"
        ? `Execution incomplete${res.reason ? `: ${res.reason}` : "."}`
        : (res?.reason ?? "The agent finished without a response.");
  finish(stream, partial, st, st.acc || fallbackText);
  return "terminal";
}
...
function streamRunViaSse(
  ...
  return new Promise((resolve) => {
    ...
    es.addEventListener("partial", (e: MessageEvent) => {
      established = true;
      try {
        const d = JSON.parse(e.data) as { partial?: string; attempt?: number; reset?: boolean };
        if (typeof d.attempt === "number" && d.attempt > 0 && d.attempt !== st.attempt) {
          if (st.attempt !== undefined && d.attempt > st.attempt) resetPartial(stream, partial, st);
          st.attempt = d.attempt;
        }
        if (d.reset === true) resetPartial(stream, partial, st);
    ...
    es.addEventListener("activity", (e: MessageEvent) => {
      established = true;
      try {
        const d = JSON.parse(e.data) as { activity?: unknown[]; startedAt?: number | null; attempt?: number };
        if (typeof d.attempt === "number" && d.attempt > 0) {
          if (st.attempt !== undefined && d.attempt > st.attempt) resetPartial(stream, partial, st);
          st.attempt = Math.max(st.attempt ?? 0, d.attempt);
        }
        const work = (partial as AssistantWork).work;
        const beforeActivity = work?.activity.length ?? 0;
        if (mergeWork(work, d)) {
...
function fail(stream: AssistantMessageEventStream, partial: AssistantMessage, errorMessage: string): void {
  ...
  if (work) {
    work.status = "failed";
    work.finishedAt = Date.now();
    work.reason = errorMessage;
    work.completionStatus = "error";
  }
...
function resetPartial(stream: AssistantMessageEventStream, partial: AssistantMessage, st: Acc): void {
  const block = partial.content[0];
